...
static void 
test_bfs_free(void *data)
{
	void *val = NULL;
	val = ((lgs_bfs_vtx *) data)->data;
	lgs_free(val);
	lgs_free(data);
}

static int 
test_bfs_match(void *key1, void *key2)
{
	return lgs_strcmp(((lgs_bfs_vtx *) key1)->data, ((lgs_bfs_vtx *) key2)->data);
}

static void 
test_bfs_tranverse(lgs_graph *g) 
{
	lgs_list *adjacent;
	lgs_bfs_vtx *vtx;
...
TestBfsCase(CuTest *tc) 
{
	...
	lgs_graph g;
	lgs_bfs_vtx *netid, netid1, *netid2;
	lgs_list *list= NULL;
	...
	lgs_graph_init(&g, test_bfs_match, test_bfs_free);

	// insert vertex
	for (i = 0; i < LGS_ARRAY_SIZE(netid_name); ++ i) {
		netid = lgs_malloc(sizeof(lgs_bfs_vtx));
	...
	for (i = 0; i < LGS_ARRAY_SIZE(net_relations); ++ i) {
		netid2 = lgs_malloc(sizeof(lgs_bfs_vtx));
	...
	test_bfs_tranverse(&g);

	lgs_print(stdout, "Generating the breadth-first search listing\n");
	list = lgs_list_create(NULL);
	lgs_strcpy(netid1.data, "node1");

	CuAssertTrue(tc, lgs_graph_bfs(&g, &netid1, list));

	i = 0;
	lgs_list_for_each(list, node) {
		netid = lgs_list_data(node);
		lgs_print(stdout, "list[%d]=%s (color=%d, hops=%d)\n", 
				i ++, (char *)netid-> data, netid->color, netid->hops);
	}
...
CuSuite *CuBfsSuite() 
{
	...
	SUITE_ADD_TEST(suite, TestBfsCase);
